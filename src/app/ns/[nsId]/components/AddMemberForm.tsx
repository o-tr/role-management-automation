...
export default function AddMemberForm({ groupId, externalProviders }: AddMemberFormProps) {
  const [name, setName] = useState("")
  const [email, setEmail] = useState("")
  const [externalAccounts, setExternalAccounts] = useState<Record<number, string>>({})
      ...
      {externalProviders.map(provider => (
        <div key={provider.id}>
          ...
          <Input
            ...
            onChange={(e) => setExternalAccounts(prev => ({ ...prev, [provider.id]: e.target.value }))}
            required
          />
        </div>
      ))}

import type { GetExternalServiceAccountsResponse } from "@/app/api/ns/[nsId]/service-accounts/route";
...
export const useServiceAccounts = (nsId: string) => {
  ...
  const fetchData = useCallback(async () => {
    const response = (await fetch(`/api/ns/${nsId}/service-accounts/`).then(
      (res) => res.json(),
